...
这对训练曲线意味着什么。四个臂训的都是同样十六个冻结 id，但**行数**因模式而异——`multi` 为每个
(harness, task) 对建一行，`single` 每个 task 只建一行——所以在固定步数下，两个臂经历的 epoch 数
差别很大。日志里的 `epoch` 字段给出确切值：
...
**这条关系是单调的，而它就是这里有用的发现。** 在同样的十六个 id 上跑更多 epoch，意味着更大比例的
GRPO 组没有组内方差，也就是没有梯度：2 个 epoch 时 26.6%，升到 16 个 epoch 时的 52.3%。这四个点
**无法**确定函数形式——线性拟合（R²=0.975）与 log₂ 拟合（R²=0.982）都描述得很好，因为 epoch 取值
恰好是连续的 2 的幂。所以诚实的说法是「单调上升」，而不是某条速率定律。

本节早前的一个版本写着「两个 128 步的臂训的是 64 行……4.0 个 epoch」。这对 `single` 臂是错的：它只有
16 行，达到 16.0 个 epoch——四倍。`single-s128` 正是那个在最少的数据上花了最多 epoch 的臂，也是浪费
梯度最多的臂。

**没有**声称的是：0.5B 模型跑 16 个 epoch 并不显然足以饱和，所以这是「这套配置停止改进了」的证据，
不是「任务集是唯一原因」的证明。而回路修复所确立的是：梯度从未见过任何一个生成任务，这无论平台
意味着什么都是缺陷。
...
**第一次实测运行发现的是这个报告自身的缺陷，而抓住它的是产物本身。** 池化后的 delta 返回负值
（`-0.0031`），脚本给出的判词是「steering 规则把任务往错误方向移动」。并不是。逐任务拆开后，
**8 个被替换的任务全部是非负**（三个 `+0.0033`，五个恰好 `0`），而整个负值来自三个**根本没人碰过**的任务：

| 任务 | 是否被移动 | before | after | Δα-reward |
| --- | --- | --- | --- | --- |
| `t2-4f700c89` | 是 | 0/256 | 1/256 | **+0.0033** |
| `t4-57df3039` | 是 | 0/256 | 1/256 | **+0.0033** |
| `t1-b6c10153` | 是 | 0/256 | 1/256 | **+0.0033** |
| `t1-9ea9b3bc` | 否 | 58/256 | 54/256 | **−0.0398** |
| `t1-8f87ad9e` | 否 | 11/256 | 10/256 | **−0.0044** |
| `t3-7e98aba7` | 否 | 1/256 | 0/256 | **−0.0033** |

被保留下来的任务不属于「处理组」。它们是**同一个任务被测了两次**，所以它们的位移天然构成一个
对照组——`n=64` 下的噪声地板——把它池化进头条数字，等于让采样噪声决定那个「被当作缺陷报告」的
数字的符号。一个没被任何干预、自己从 `58/256` 漂到 `54/256` 的任务，不是关于 steering 规则的证据。

报告现在计算并打印三个数字，并把判词建立在中间那个上：

| 数字 | 它是什么 |
| --- | --- |
| `alignment_delta` | 池化值——保留它，是为了与打印出来的表格连续 |
| `alignment_delta_moved` | **处理效应**——steering 规则真正要负责的那个数 |
| `alignment_delta_kept` | 对照组——处理效应必须超过的尺度，永远不是结论 |

池化值与处理效应符号不一致时，现在会被**明确写出来**，而不是被悄悄地在两者中挑一个。在真实运行中，
脚本打印如下内容并以 `0` 退出：
...
**所以对第一次闭环最诚实的读法是：方向是对的，步子太小。** 8 次移动全部是
`out_of_reach → easier`，正是 band 定义要求的方向。但批次没有动——`out_of_reach 11 → 11`、
`frontier 1 → 1`——而且处理效应（`+0.0012`）比对照组噪声（`-0.0119`）**低一个数量级**。
也就是说在这个 `n` 下，steering 规则与噪声地板不可区分，而说明这一点的那个量是**步长**
（每个旋钮一步），不是方向。`band.steer` 把 `payload_len` 减半、`escape_density` 降 `0.2`、
`steps` 减一——这不足以把一个在**最易参数设置**下 0.5B 策略都只有 0/256 的任务抬离地板。
